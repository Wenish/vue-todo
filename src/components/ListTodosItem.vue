<template>
  <li>
    {{ todo.title }}
    <div class="buttons">
      <button class="remove" v-on:click="onDeleteTodo">
        <svg
          version="1.1"
      ...
      <button class="complete" v-on:click="onToggleTodo">
        <svg
          version="1.1"
          xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink"
...
export default {
  ...
  props: ["todo"],
  methods: {
    ...mapActions(["todoDelete", "todoToggle"]),
    onDeleteTodo() {
      this.todoDelete(this.todo.id);
    },
    onToggleTodo() {
      this.todoToggle(this.todo.id);
    },
  },
};
</script>
